...
//var timeNow = Date().add(3,TimeUnits.HOUR).time
    ...
    state += when (time) {
        ...
        in 75..45*60 -> "через ${time/ 60} ${if( (time/ 60 )>=2&&(time/ 60 )<=4 ) "минуты" else "минут"}"
        in 45*60..75*60 -> "через час"
        in 75*60..22*60*60  -> "через ${time/60/60} ${if( (time/ 60 / 60 ) in 2..4) "часа" else "часов"}"
        ...
        in -45*60..-75 -> "${time / 60 *(-1)} ${if( (time/ 60*(-1 )) in 1..4) " минуты назад" else " минут назад"}"
        in -75*60..-45* 60 -> "час назад"
        in -22* 60*60..-75* 60  -> "${time/60/60*(-1)}${if( (time/ 60 / 60 *(-1)) in 1..4) " часа назад" else " часов назад"}"
        in  -26* 60*60..-22*60*60  ->  "день назад"
        in -360* 60*60..-22* 60*60 -> "${time/ 60/60/24*(-1)} ${if( (time/ 60 / 60 / 24*(-1)) in 2..4)"дня назад" else "дней назад"}"
